...
import { applyColorKey, computeOpaqueBounds, detectOpaqueComponents, guessBackgroundColor, type ColorKeyOptions, type DetectComponentsOptions, type EraseStroke, type ImageOpRequest, type ImageOpResponse } from "./ops";
...
function bgColorFromBitmap(bitmap: ImageBitmap) {
  const canvas = new OffscreenCanvas(bitmap.width, bitmap.height);
  const ctx = canvas.getContext("2d")!;
  ctx.drawImage(bitmap, 0, 0);
  const imageData = ctx.getImageData(0, 0, bitmap.width, bitmap.height);
...
async function colorKeyFromBitmap(bitmap: ImageBitmap, options: ColorKeyOptions) {
  const canvas = new OffscreenCanvas(bitmap.width, bitmap.height);
  const ctx = canvas.getContext("2d")!;
...
self.onmessage = async (e: MessageEvent<ImageOpRequest>) => {
  const { id, op, blob, rect, strokes, quarterTurns, flipHorizontal, componentOptions, colorKeyOptions } = e.data;
  ...
  try {
    ...
    if (op === "bounds") {
      ...
      if (!colorKeyOptions) throw new Error("colorkey 缺少 colorKeyOptions");
      const out = await colorKeyFromBitmap(bitmap, colorKeyOptions);
      postMessage({ id, ok: true, blob: out });
    } else if (op === "bgcolor") {
      postMessage({ id, ok: true, color: bgColorFromBitmap(bitmap) });
    } else if (op === "edit") {
